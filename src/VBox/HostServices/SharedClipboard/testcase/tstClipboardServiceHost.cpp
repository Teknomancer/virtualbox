...
/* Generated by VBoxDef2LazyLoad from the libX11.def and libXt.def files. */
extern "C" int ExplicitlyLoadlibX11(bool fResolveAllImports, PRTERRINFO pErrInfo);
extern "C" int ExplicitlyLoadlibXt(bool fResolveAllImports, PRTERRINFO pErrInfo);

static int tstLoadX11Libraries(const char **ppcszLibrary, PRTERRINFO pErrInfo)
{
    *ppcszLibrary = "libX11";
    int rc = ExplicitlyLoadlibX11(false /* fResolveAllImports */, pErrInfo);
    if (RT_FAILURE(rc))
        return rc;

    *ppcszLibrary = "libXt";
    return ExplicitlyLoadlibXt(false /* fResolveAllImports */, pErrInfo);
}
#endif
...
#ifdef VBOX_WITH_SHARED_CLIPBOARD_X11_LAZY_LOAD
    const char *pcszLibrary = NULL;
    RTERRINFOSTATIC ErrInfo;
    int rc = tstLoadX11Libraries(&pcszLibrary, RTErrInfoInitStatic(&ErrInfo));
    if (RT_FAILURE(rc))
        return RTTestSkipAndDestroy(hTest, "Skipping: cannot load %s for the X11 clipboard backend: %Rrc%s%s",
                                    pcszLibrary, rc,
